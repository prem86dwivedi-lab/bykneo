...
export const LiveTrackingScreen = ({ ride, onCancelRide }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-20 px-2.5 pb-2.5 pb-[env(safe-area-inset-bottom,10px)] pointer-events-none">
      <div className="max-w-md mx-auto bg-gray-900/95 backdrop-blur-xl border border-gray-800 rounded-2xl shadow-2xl pointer-events-auto overflow-hidden">

        {/* ── Drag Handle ── */}
        <div
          className="flex justify-center pt-1.5 pb-0.5 cursor-pointer"
        ...
        <div className="px-3 pb-3 space-y-2">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="relative flex h-2.5 w-2.5 shrink-0">
                ...
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-brand-yellow" />
              </span>
              <span className="text-[11px] font-black uppercase tracking-wide text-brand-yellow truncate">
                {isInProgress
                  ? '🚀 Trip in Progress to Drop'
                  : isArrived
                  ? '📍 Captain Arrived at Pickup!'
                  : '🏍️ Captain Approaching Pickup'}
              </span>
            </div>
            <span className="text-[10.5px] font-bold text-gray-300 bg-gray-800 px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0">
          ...
          {/* ── Start Ride OTP Card ── */}
          {!isInProgress && (
            <div className="bg-gradient-to-r from-amber-500/15 via-yellow-500/10 to-amber-500/15 border border-brand-yellow/40 p-2 rounded-xl flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-brand-yellow text-gray-950 flex items-center justify-center font-black shadow-md shrink-0">
                ...
                <div>
                  ...
                  <div className="text-base font-black text-brand-yellow tracking-widest leading-tight mt-0.5">
                    {ride.otp || '4829'}
                  </div>
                </div>
              </div>
              <div className="text-right">
                <span className="text-[9px] text-gray-400 block font-bold uppercase leading-none">Est. Fare</span>
                <span className="text-sm font-black text-brand-yellow leading-tight mt-0.5">₹{ride.fare}</span>
              </div>
            </div>
          )}
          ...
          <div className="bg-gray-850 p-2 rounded-xl border border-gray-800 flex items-center justify-between shadow-sm">
            ...
            <div className="flex items-center gap-1.5 shrink-0 ml-2">
              <button
                ...
                className="relative w-8 h-8 rounded-xl bg-brand-yellow/15 hover:bg-brand-yellow/25 border border-brand-yellow/40 flex items-center justify-center text-brand-yellow shadow active:scale-95 transition"
              ...
              <a
                ...
                className="w-8 h-8 rounded-xl bg-gray-800 hover:bg-gray-750 border border-gray-700 flex items-center justify-center text-emerald-400 shadow active:scale-95 transition"
                title="Call Captain"
              >
              ...
              <button
                ...
                className="w-8 h-8 rounded-xl bg-red-500/20 hover:bg-red-500/30 border border-red-500/40 flex items-center justify-center text-red-400 shadow active:scale-95 transition"
                title="Emergency SOS"
              >
          ...
          <button
            onClick={() => setIsExpanded(v => !v)}
            className="w-full flex items-center justify-between bg-gray-850/80 border border-gray-750 rounded-xl px-2.5 py-1.5 text-[11px] active:scale-[0.98] transition-transform"
          ...
          >
            ...
            <span
              ...
              className="text-[10px] font-bold text-brand-yellow flex items-center gap-1 bg-brand-yellow/10 border border-brand-yellow/30 px-2 py-0.5 rounded-lg active:scale-95 transition"
            >
          ...
          >
            <div className="space-y-2 pt-1">
              ...
              <div className="bg-gray-850/80 rounded-xl px-2.5 py-2 border border-gray-800 space-y-1.5 text-[10.5px]">
                <div className="flex items-start gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 mt-1 shrink-0" />
                  <div className="min-w-0">
                    <span className="text-[9px] font-bold text-gray-400 uppercase block leading-none">Pickup Location</span>
                    <span className="text-white font-medium truncate block mt-0.5">{ride.pickup_name}</span>
                  </div>
                </div>
                <div className="flex items-start gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-red-400 mt-1 shrink-0" />
                  <div className="min-w-0">
                    <span className="text-[9px] font-bold text-gray-400 uppercase block leading-none">Drop Destination</span>
                    <span className="text-white font-medium truncate block mt-0.5">{ride.drop_name}</span>
                  </div>
                </div>
              </div>
              ...
              {!isInProgress && (
                <button
                  ...
                  className="w-full py-2 rounded-xl bg-gray-850 hover:bg-gray-800 text-red-400 text-[11px] font-bold transition active:scale-95 border border-red-500/20"
                >
                  Cancel Ride
                </button>
              )}
