...
export const CaptainKycScreen = ({ driverProfile, onBack, onKycSubmitted }) => {
  ...
  // Wizard Step State (1 = Vehicle & Docs, 2 = Selfie & UPI Payout)
  ...
  const handleContinueToStep2 = (e) => {
    ...
    if (!vehicleModel?.trim() || !vehicleNumber?.trim() || !licenseNumber?.trim() || !rcNumber?.trim() || !aadhaarNumber?.trim()) {
      setStatusMessage({
        type: 'error',
        text: 'Please complete all vehicle and document details to continue to Step 2.'
      });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-2.5 pb-16 overflow-y-auto">
      <div className="max-w-md mx-auto space-y-2.5">
        ...
        <div className="flex items-center justify-between pt-0.5">
          ...
          {/* Quick Step Badge */}
          <span className="text-[9px] font-black text-brand-yellow bg-brand-yellow/10 border border-brand-yellow/30 px-2 py-0.5 rounded-full shrink-0">
          ...
          <button
            ...
            className={`py-1.5 px-2 rounded-xl transition flex items-center justify-center gap-1.5 ${
              currentStep === 1
                ...
                : 'bg-gray-850/80 text-gray-300 hover:text-white'
            }`}
          >
            <div
              className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black ${
                currentStep === 1
                  ? 'bg-gray-950 text-brand-yellow'
                  : isStep1Complete
                  ? 'bg-emerald-500 text-white'
                  : 'bg-gray-750 text-gray-400'
              }`}
            >
              {isStep1Complete && currentStep !== 1 ? '✓' : '1'}
            </div>
            <span className="text-[9.5px] uppercase tracking-wider truncate">
              1. Vehicle & Docs
            </span>
          </button>
          ...
          <button
            ...
            onClick={() => {
              if (isStep1Complete) {
                ...
                setStatusMessage({
                  type: 'error',
                  text: 'Please complete Step 1 vehicle and document details first.'
                });
              }
            }}
            className={`py-1.5 px-2 rounded-xl transition flex items-center justify-center gap-1.5 ${
        ...
        {/* ============================================================ */}
        {/* STEP 1: VEHICLE TYPE & GOVERNMENT DOCUMENTS SCREEN           */}
        {/* ============================================================ */}
        {currentStep === 1 && (
          <div className="space-y-2.5 animate-in fade-in slide-in-from-left duration-200">
            ...
            {/* Step 1 Continue Button (Right Flip to Step 2) */}
            <button
              type="button"
              onClick={handleContinueToStep2}
              className="w-full py-3 bg-brand-yellow hover:bg-brand-yellowHover text-gray-950 font-black text-xs rounded-xl shadow-xl shadow-brand-yellow/20 active:scale-[0.98] transition flex items-center justify-center gap-2"
            >
              <span>CONTINUE TO LIVE SELFIE & PAYOUT (STEP 2)</span>
        ...
        {/* ============================================================ */}
        {/* STEP 2: LIVE SELFIE & DIRECT PAYOUT SCREEN (Right Flip View) */}
        {/* ============================================================ */}
        {currentStep === 2 && (
          <form onSubmit={handleSubmit} className="space-y-2.5 animate-in fade-in slide-in-from-right duration-200">
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                type="button"
                onClick={() => setCurrentStep(1)}
                className="py-3 px-3 bg-gray-850 hover:bg-gray-800 border border-gray-750 text-gray-300 font-bold text-xs rounded-xl active:scale-95 transition flex items-center gap-1 shrink-0"
              >
                <ArrowLeft className="w-3.5 h-3.5" />
                <span>Back</span>
              </button>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="flex-1 py-3 bg-brand-yellow hover:bg-brand-yellowHover text-gray-950 font-black text-xs rounded-xl shadow-xl shadow-brand-yellow/20 active:scale-[0.98] transition flex items-center justify-center gap-1.5 disabled:opacity-50"
              >
                <Upload className="w-3.5 h-3.5" />
                <span>{submitting ? 'Submitting Documents...' : 'SUBMIT DOCUMENTS FOR VERIFICATION'}</span>
              </button>
            </div>
          </form>
        )}
