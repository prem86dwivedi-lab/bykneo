              ...
              <div className="space-y-3 animate-in fade-in">
                ...
                {!successMsg && (
                  <div className="space-y-2.5 pt-0.5">
                    {/* CASE 1: Both Payment Methods are Disabled by Admin */}
                    {(subData?.gateway_enabled === false && subData?.direct_upi_qr_enabled === false) ? (
                      <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-center space-y-2.5 my-2 animate-in zoom-in-95">
                        <div className="w-10 h-10 rounded-xl bg-amber-500/20 text-brand-yellow flex items-center justify-center mx-auto">
                          <AlertCircle className="w-5 h-5" />
                        </div>
                        <div className="text-sm font-black text-white">Payment Method is Currently Unavailable</div>
                        <p className="text-xs text-gray-300 leading-relaxed max-w-xs mx-auto">
                          Online pass payment methods have been turned OFF by the Administrator. Please contact admin directly or check back shortly.
                        </p>
                      </div>
                    ) : (
                      <>
                        {/* CASE 2: Payment Gateway Option (Razorpay / Automated Checkout) */}
                        {subData?.gateway_enabled !== false && (
                          <div className="space-y-1.5">
                            <button
                        ...
                        {/* CASE 3: Direct Admin Bank UPI & QR Settlement Option */}
                        {subData?.direct_upi_qr_enabled !== false && (
                          <div className={`pt-1.5 ${subData?.gateway_enabled !== false ? 'border-t border-gray-800' : ''}`}>
                            {subData?.gateway_enabled !== false ? (
                              <button
                                type="button"
                            ...
                            {(subData?.gateway_enabled === false || showDirectUpi) && (
