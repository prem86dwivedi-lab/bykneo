...
export const getRideMessages = (req, res) => {
  ...
  if (!ride) return res.status(404).json({ success: false, error: 'Ride not found' });
  return res.json({ success: true, messages: Array.isArray(ride.messages) ? ride.messages : [] });
};

export const sendRideMessage = (req, res) => {
  const { id } = req.params;
  const { text, senderRole, senderName, senderId, id: clientMsgId, time, timestamp } = req.body;
  const ride = db.find('rides', r => String(r.id) === String(id));
  if (!ride) return res.status(404).json({ success: false, error: 'Ride not found' });
  ...
  const msgId = clientMsgId || `msg_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`;
  const messageObj = {
    id: msgId,
    rideId: ride.id,
    senderId: senderId || 'usr_unknown',
    senderRole: senderRole || 'rider',
    senderName: senderName || (senderRole === 'driver' ? (ride.driver_name || 'Captain') : (ride.rider_name || 'Rider')),
    text: text.trim(),
  ...
  if (!existingMessages.some(m => m.id === messageObj.id)) {
